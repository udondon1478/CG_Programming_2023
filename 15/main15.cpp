...
void timer(int value)
{
    ...
    if (armAngle > -40 && direction == -1)
    {
        armAngle -= 1;
    }
    else if (armAngle < 30 && direction == 1)
    {
        armAngle += 1;
    }
    ...
    //armAngleとは違い、swingは指定された値になるにつれてゆっくり動き、指定された値になったら逆方向に動くコード
        if (swing > -45 && swingDirection == -1)
    {
        swing -= 1;
    }
    else if (swing < 45 && swingDirection == 1)
    {
        swing += 1;
    }
...
void drawArm()
{
    ...
    glRotated(armAngle, 0.0, 1.0, 0.0);
    ...
    glRotated((armAngle+20)*1.5, 0.0, 1.0, 0.0);
    glPushMatrix();
...
void drawFerrisWheel()
{
    ...
    glPushMatrix();
    glRotated(theta * 0.5, 0, 0, 1);

    // 中心の軸
    glMyCylinder(10, 10, 20, 20);
    glMyCylinder(5, 5, 30, 20);
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        glTranslated(0, 0, -8);
        glRotated(90.0, 1.0, 0.0, 0.0);
        glRotated(360.0 / 8.0 * i, 0.0, 1.0, 0.0);
        glMyCylinder(1, 1, 200, 20);
        glPopMatrix();
    }
    glPopMatrix();
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            glPushMatrix();
            // 円の外周に8個の軸を描画
            glTranslated(0, -8, 100);
            glRotated(90.0, 1.0, 0.0, 0.0);
            glMyCylinder(3, 3, 20, 20);

            // 軸の下に乗り場を描画
            glPushMatrix();
            // 変数jが一つ増えるたびに45度傾けた位置に移動
            glRotated(360.0 / 8.0 * i, 0.0, 0.0, 1.0);
            glRotated(theta * 0.5, 0, 0, 1);
            glTranslated(0, 13, 0);
            // 色をランダムに設定
            col[0] = rand() % 100 / 100.0;
            col[1] = rand() % 100 / 100.0;
            col[2] = rand() % 100 / 100.0;
            col[3] = 1.0;
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, col);

            glMyCylinder(10, 10, 15, 20);
            glPopMatrix();
            glPopMatrix();
        }
    ...
    glPushMatrix();
    glTranslated(0, -70, -30);
    glRotated(-75.0, 1.0, 0.0, 0.0);
    glMyCylinder(2, 2, 145, 20);
...
void drawSwing()
{
    // 棒に振り子運動をさせる
    glPushMatrix();
    glRotated(swing, 0.0, 0.0, 1.0);
    glPushMatrix();
    glMyCylinder(2, 2, 50, 20);
    glPushMatrix();
    glRotated(90.0, 1.0, 0.0, 0.0);
    glTranslated(0, 25,25);
    glMyCylinder(2, 2, 100, 20);
    glPopMatrix();
    glPushMatrix();
    glRotated(90.0, 1.0, 0.0, 0.0);
    glTranslated(0, -25,25);
