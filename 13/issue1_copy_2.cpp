...
int fID = 0;                 // フレームID
double bottom = -800;        // 床の高さ

double r[BALLNUM]; // ボールの半径
Vec_3D p[BALLNUM]; // ボールの位置
Vec_3D v[BALLNUM]; // ボールの速度
Vec_3D c[BALLNUM]; // ボールの色
...
void display()
{
    ...
    for (int k = 0; k < BALLNUM; k++)
    {
        ...
        glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, col2); // 拡散反射
        glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, spe2);            // 鏡面反射
        glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, shi2);           // ハイライト
        // 配置
    ...
    */
    ...
    // ifの条件をorに変更
    if (fID == 66 || fID == 100 || fID == 140 || fID == 200 || fID == 240)
    {
        srand((unsigned)time(NULL)); // 乱数初期化
        for (int k = 0; k < BALLNUM; k++)
        {
