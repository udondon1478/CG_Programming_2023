    ...
    void drawTrail()
    {
        ...
        for (int i = 0; i < trailCount; i++)
        {
            ...
            for (int j = 0; j < 1500; j++)
            {
                glPushMatrix();
                //だんだん小さくする
                //中心点をもう少し前のフレームのものにすると、残像がより滑らかになる
                //中心点をもう少し前のフレームのものにする処理を加える
                glTranslated(jData[(fID - i - 1 + FMAX) % FMAX][j].x, jData[(fID - i - 1 + FMAX) % FMAX][j].y, jData[(fID - i - 1 + FMAX) % FMAX][j].z - 0.0);
                glScaled(30.0 * (1.0 - 0.1 * i), 30.0 * (1.0 - 0.1 * i), 30.0 * (1.0 - 0.1 * i));
