...
void display0()
{
    ...
    //----------床----------
    //色の設定
    col[0] = 0.0; col[1] = 0.7; col[2] = 0.7;  col[3] = 1.0;  //拡散反射係数
    ...
    //オブジェクト
    //色の設定
    col[0] = 1.0; col[1] = 0.2; col[2] = 0.2;  col[3] = 1.0;  //拡散反射係数
    spe[0] = 1.0; spe[1] = 1.0; spe[2] = 1.0; spe[3] = 1.0;  //鏡面反射係数
    shi[0] = 100.0;  //ハイライト係数
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, col);  //拡散反射，環境光反射設定
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, spe);  //鏡面反射設定
...
void motion(int x, int y)
{
    ...
    if (mButton==GLUT_RIGHT_BUTTON) {  //右ボタンドラッグのとき
        //マウスの移動量を角度変化量に変換
    ...
    else if (mButton==GLUT_LEFT_BUTTON) {  //左ボタンドラッグのとき
        //マウス座標からウィンドウ座標の取得
        winX = x; winY = winH-y;  //x座標，y座標
