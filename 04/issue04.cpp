...
void kirby(double x, double y, double scale);

// グローバル変数
int px[MAX_OBJECT_NUM] = {0.0}, py[MAX_OBJECT_NUM] = {0.0};
int winW = 800, winH = 600;
double r[MAX_OBJECT_NUM] = {50.0};
int N[MAX_OBJECT_NUM] = {24};
...
int scale_flag[MAX_OBJECT_NUM] = {0};     // 拡大・縮小ON/OFF用フラグ．デフォルトはOFF
int rotate_flag[MAX_OBJECT_NUM] = {0};    // 回転ON/OFF用フラグ．デフォルトはOFF
int translate_flag[MAX_OBJECT_NUM] = {0}; // 横揺れON/OFF用フラグ．デフォルトはOFF
int object_type[MAX_OBJECT_NUM] = {0};    // オブジェクトの種類記憶用変数

int object_num = 0; // オブジェクトの数
...
void display()
{
    ...
    for (int i = 0; i < object_num; i++)
    {
        ...
        {
        case 1:
            triangle(px[i], py[i], r[i]); // オリジナルオブジェクト描画関数○○○の呼び出し
            break;
        case 2:
            astral(px[i], py[i], r[i]);
            break;
        case 3:
            kirby(px[i], py[i], r[i]);
            break;
        default:
            glMyWireCircle2(px[i], py[i], r[i], 3.0, N[i]);
            break;
        }
        glPopMatrix(); // 動き制御終わり
    }
...
void mouse(int button, int state, int x, int y)
{
    ...
    if (button == GLUT_LEFT_BUTTON)
    {
        if (state == GLUT_DOWN)
        {
            px[object_num] = x - winW / 2.0;
            py[object_num] = winH / 2.0 - y;
            r[object_num] = 1.0;
            object_num++; // 押したときにオブジェクトが増えるので，インクリメント
            printf("Number of Objects = %d\n", object_num);
            // glutPostRedisplay();
        }
        if (state == GLUT_UP)
        {
            r[object_num - 1] = sqrt((x - mX) * (x - mX) + (y - mY) * (y - mY));
            // glutPostRedisplay();
        }
    }
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    case '0':
        scale_flag[object_num] = (scale_flag[object_num] + 1) % 2; // トグルスイッチ
        break;
    case '1':
        rotate_flag[object_num] = (rotate_flag[object_num] + 1) % 2; // トグルスイッチ
        break;
    case '2':
        translate_flag[object_num] = (translate_flag[object_num] + 1) % 2; // トグルスイッチ
        break;
    case 't':
        object_type[object_num] = 1;
        break;
    case 'T':
        object_type[object_num] = 1;
        break;
    case 'a':
        object_type[object_num] = 2;
        break;
    case 'A':
        object_type[object_num] = 2;
        break;
    case 'k':
        object_type[object_num] = 3;
        break;
    case 'K':
        object_type[object_num] = 3;
        break;
    case 'c':
        object_type[object_num] = 0;
        break;
    case 'C':
        object_type[object_num] = 0;
        break;
    ...
    case 127:
        object_num--; // やり直し
        if (object_num < 0)
            object_num = 0; // 負になると配列外へアクセスして落ちる
        printf("Number of Objects = %d\n", object_num);
        break;
    default:
        break;
