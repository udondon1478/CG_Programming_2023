...
// マウスをクリックしている間、周囲のボールを引き寄せる機能を追加
...
void drawString2(char *str, int w, int h, int x0, int y0);
...
int attract_x, attract_y, attract_z; // 引き寄せる座標
...
int attract_flag = 0;
// attract_modeに文字列を格納
char attract_mode[100] = "attract_mode: off";

int heightFlag = 0; // 0:平面 1:山 2:谷
...
void display()
{
    ...
    */
    ...
    for (int k = 0; k < BALLNUM; k++)
    {
        // ATTRACT_RADIUSは引力の半径、この範囲内にあるボールが引き寄せられる
        double grav_rad = ATTRACT_RADIUS;
        // 引力の強さ
        double grav_pow = ATTRACT_POWER;

        // 引力の発生する座標
        Vec_3D attract = {attract_x, attract_y, attract_z};
        Vec_3D distVec = vectorDiff(attract, p[k]);
        double distance = vectorNormalize(&distVec);
        if (distance < grav_rad && mButton == GLUT_LEFT_BUTTON && attract_flag == 1)
        {
            v[k].x += distVec.x * grav_pow;
            v[k].y += distVec.y * grav_pow;
            v[k].z += distVec.z * grav_pow;
            // 位置を更新
            p[k].x += v[k].x / fr;
            p[k].y += v[k].y / fr;
            p[k].z += v[k].z / fr;
        ...
        /*
        //attract_x,attract_y,attract_zはドラッグした際の座標
        Vec_3D attract = { attract_x, attract_y, attract_z };
        Vec_3D distVec = vectorDiff(attract, p[k]);
        double distance = vectorNormalize(&distVec);
        if (distance < 100) {
    ...
    // attract_flagが1の時attract_modeに"ON"を格納
    if (attract_flag == 1)
    {
        strcpy(attract_mode, "ON");
    }
    else
    {
        strcpy(attract_mode, "OFF");
    }

    char str[256];
    sprintf(str, "attract_mode: %s", attract_mode);
    drawString(str, winW, winH, 10, 10); // 文字列描画

    char str2[256];
    sprintf(str2, "Terrain_mode:%d", heightFlag);
    drawString(str2, winW, winH, 10, 30); // 文字列描画

    // クリックした地点にブロックを生成する
    glBegin(GL_QUADS);
    glColor3d(1.0, 0.0, 0.0);
    glVertex3d(attract_x, attract_y, attract_z);
    glVertex3d(attract_x, attract_y, attract_z + 10);
    glVertex3d(attract_x + 10, attract_y, attract_z + 10);
    glVertex3d(attract_x + 10, attract_y, attract_z);
    glEnd();
