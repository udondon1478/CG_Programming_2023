//g++ -O3 moyac.cpp -framework OpenGL -framework GLUT  -Wno-deprecated
...
void initGL()
{
   ...
   for (int j=0; j<TILE; j++) {
    ...
    //球初期値/
    r = 30.0; //半径
    p.x = 0.0; p.y = bottom+r+100; p.z = 0.0; //初期座標
    v.x = 150.0; v.y = 0.0; v.z = 200.0; //初期速度
...
void display()
{
   ...
   //----------球----------
    ...
    spe[0] = 0.0; spe[1] = 0.0; spe[2] = 0.0; spe[3] = 1.0;  // 鏡面反射係数
    shi[0] = 100.0;  // ハイライト係数
   ...
   glPopMatrix();  //一時保存した行列の復帰
    ...
    // 色の設定
    col[0] = 0.0; col[1] = 0.0; col[2] = 0.0; col[3] = 1.0;  // 拡散反射係数
    spe[0] = 0.0; spe[1] = 0.0; spe[2] = 0.0; spe[3] = 1.0;  // 鏡面反射係数
    shi[0] = 100.0;  // ハイライト係数
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, col);  // 拡散反射
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, spe);  // 鏡面反射
    ...
    //球の位置更新
    p.x += v.x/fr; p.y += v.y/fr; p.z += v.z/fr;
    ...
    //x 成分
    if (p.x<-fWidth/2.0+r) {
        v.x=-v.x*at1; //速度のx成分を反転
        p.x = -fWidth/2.0+r; //めり込みを解消
    }
    if (p.x>fWidth/2.0-r) {
        v.x=-v.x*at1; //速度のx成分を反転
        p.x = fWidth/2.0-r; //めり込みを解消
    ...
    //z 成分
    if (p.z<-fWidth/2.0+r) {
        v.z=-v.z*at1; //速度のz成分を反転
        p.z = -fWidth/2.0+r; //めり込みを解消
    }
    if (p.z>fWidth/2.0-r) {
        v.z=-v.z*at1; //速度のz成分を反転
        p.z = fWidth/2.0-r; //めり込みを解消
        ...
        //y 成分
    if (p.y<bottom+r) {
        v.y=-v.y*at1; //速度のx成分を反転
        p.y = bottom+r; //めり込みを解消
        v.x *= at2; v.z *= at2; //転がりによる減衰
    }
    if(p.y>300-r){
        v.y=-v.y;
        p.y=300-r;
...
void mouse(int button, int state, int x, int y)
{
   ...
   if (state == GLUT_DOWN) {
      ...
      if (button == GLUT_LEFT_BUTTON) {
         ...
         if (distance <= (r + 5.0)) {  // 半径にマージン（5.0）を追加
            ballClicked = true;
