...
void display()
{
    ...
    for (int j = 0; j < TILE - 1; j++)
    {
        ...
        {
            //クリックした座標の半径2以内に赤い床面を表示、それ以外は緑
            if ( sqrt(pow(boxPos.x - floorPoint[i][j].x, 2) + pow(boxPos.y - floorPoint[i][j].y, 2) + pow(boxPos.z - floorPoint[i][j].z, 2)) < 2.0)
            {
                // 色の設定
                col[0] = 1.0;
                col[1] = 0.0;
